...
class handDetector:
    def __init__(self, mode=False, max_num_hands=2, detection_confidence=0.5, tracking_confidence=0.5):
        self.mode = mode
        self.maxHands = int(max_num_hands)
        self.detectionConfidence = float(detection_confidence)
        self.trackingConfidence = float(tracking_confidence)
    ...
    def fingersUp(self) :
        ...
        fingers = []
        # Thumb Detection
        if self.lmList[self.tipIds[0]][1] > self.lmList[self.tipIds[0] - 1][1]:  # for thumb we need to take the x-coordinate.
            fingers.append(1)
        else:
            fingers.append(0)
        # Other Fingers
